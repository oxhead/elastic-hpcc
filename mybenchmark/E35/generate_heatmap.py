...
def main(run_id):
    ...
    default_setting.set_config('experiment.benchmark_manual_routing_table', False)
    default_setting.set_config('experiment.roxie_concurrency', 80)
    default_setting.set_config('experiment.data_dir', '/dataset_local')
    default_setting.set_config('experiment.storage_type', 'local')
    ...
    for experiment in generate_experiments(default_setting, variable_setting_list, experiment_dir=script_dir, timeline_reuse=True, wait_time=1, check_success=True, restart_hpcc=True, timeout=300):
