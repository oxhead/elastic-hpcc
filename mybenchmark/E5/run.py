...
def main():
    ...
    default_setting.set_config('experiment.id', 'E5')
    ...
    output_dir = os.path.join(default_setting['experiment.result_dir'], default_setting['experiment.id'], "64threads_layout_1r_{}q_{}sec".format(default_setting['workload.num_queries'], default_setting['workload.period']))
    run_ids = range(1, 4)
    for run_id in run_ids:
        variable_setting = [
            ...
            {
                'experiment.application': 'search_firstname_distribution',
                'experiment.output_dir': os.path.join(output_dir, 'naivesearch_pareto_{}'.format(run_id)),
                'workload.distribution': {"type": "pareto", "alpha": 3},
            },
            {
                'experiment.application': 'search_firstname_distribution',
                'experiment.output_dir': os.path.join(output_dir, 'naivesearch_uniform_{}'.format(run_id)),
                'workload.distribution': {"type": "uniform"},
            },
        ]
