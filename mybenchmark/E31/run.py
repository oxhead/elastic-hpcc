...
def main(dp_type, dp_model, selective_run, check_success, num_iters=1):
    ...
    #####################################
    configuration_table = {
        4: '/home/chsu6/elastic-hpcc/template/elastic_1thor_4roxie_locality_nfs.xml',
        5: '/home/chsu6/elastic-hpcc/template/elastic_1thor_5roxie_locality_nfs.xml',
        6: '/home/chsu6/elastic-hpcc/template/elastic_1thor_6roxie_locality_nfs.xml',
        7: '/home/chsu6/elastic-hpcc/template/elastic_1thor_7roxie_locality_nfs.xml',
        8: '/home/chsu6/elastic-hpcc/template/elastic_1thor_8roxie_locality_nfs.xml',
        12: '/home/chsu6/elastic-hpcc/template/elastic_1thor_12roxie_locality_nfs.xml',
        16: '/home/chsu6/elastic-hpcc/template/elastic_1thor_16roxie_locality_nfs.xml',
    }
    ...
    default_setting.set_config('cluster.deploy_config', True)
    default_setting.set_config('cluster.benchmark', "/home/chsu6/elastic-hpcc/conf/benchmark_template.yaml")
    default_setting.set_config('experiment.id', 'E31')
    default_setting.set_config('experiment.goal', '''
    1. This test uses only 128 partitions
    2. Compare with E26, which has 1024 partitions
    ''')
    default_setting.set_config('experiment.conclusion', '''
    1. Seems similar results
    ''')
    ...
    for workload, data_placement in itertools.product(workload_distribution_list.items(), data_placement_list.items()):
        ...
        if data_placement_name != 'complete' and workload_config['type'] != data_placement_name:
            continue

        for cluster_size in cluster_sizes:
            # this requires 8 nodes in conf/benchmark_template.yaml
            variable_setting_list.append({
                'cluster.num_nodes': cluster_size,
                'experiment.workload_endpoints': cluster_size,
                'experiment.benchmark_instances': cluster_size,
                'cluster.target': configuration_table[cluster_size],
                'experiment.output_dir': os.path.join(
                    default_setting['experiment.result_dir'],
                    default_setting['experiment.id'],
                    str(cluster_size),
                    "{}-{}".format(data_placement_type, dp_model),
                    'w-{}_dp-{}'.format(workload_name, data_placement_name)),
                'experiment.data_placement': data_placement_profile,
                'experiment.dp_name': "dp_{}_{}_{}_{}".format(dp_type, dp_model, data_placement_name, cluster_size),
    ...
    #import sys
    #sys.exit(0)
    for experiment in generate_experiments(default_setting, final_variable_setting_list, experiment_dir=script_dir, timeline_reuse=True, wait_time=1, check_success=check_success, restart_hpcc=True, timeout=600):
