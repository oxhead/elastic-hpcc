...
def main():
    ...
    output_dir = os.path.join(default_setting['experiment.result_dir'], default_setting['experiment.id'])
    ...
    # generate workloads that can be reused
    workload_records = {}
    ...
    for workload_type, workload_skew, scheme in itertools.product(workload_type_list, workload_skew_list, scheme_list):
        ...
        for N in N_list:
            ...
            for k in k_list:
                ...
                skew_records[N][k] = metric_records

        for metric in metric_list:
            output_path = os.path.join(output_dir, "M{}_{}_{}_{}.json".format(M, workload_type, scheme, metric))

            records_output = {key_N: {key_k: record[metric] for key_k, record in skew_records[key_N].items()} for key_N in skew_records.keys() }
