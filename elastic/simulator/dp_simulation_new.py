...
def run(M, N, k, t, workload_name='uniform', af_list=[], show_output=True):
    ...
    print("workload distribution:", workload_name)
    ...
    #print([str(n) for n in dp.nodes])
    #print([str(p) for p in dp.partitions])
    ...
    return dp, adjusted_replicas_list


def analyze_partition_granularity(M, N, workload_name):
    # [1, 16, 64, 128, 512, 1024]
    for num_partition in [1, 4]:
        S = N * num_partition
        af_list = WorkloadGenerator.new(workload_name, M*num_partition)
        minimum_replicas_list = generate_minimum_replica_list(af_list, S)
        minimum_weight_list = [af_list[i] / minimum_replicas_list[i] for i in range(len(af_list))]
        adjusted_replicas_list = adjust_num_replicas_by_weight(af_list, minimum_replicas_list, S)
        adjusted_weight_list = [af_list[i] / adjusted_replicas_list[i] for i in range(len(af_list))]
        #expected_load_list = [adjusted_replicas_list[n] / S for n in range(len(adjusted_replicas_list))]
        print("af_list", af_list)
        print("replica", adjusted_replicas_list)
        #print("expected", expected_load_list)
        #delta_list = [abs(af_list[n] - expected_load_list[n]) for n in range(len(af_list))]
        delta_list = [abs(af_list[n] - 1/S * adjusted_replicas_list[n]) for n in range(len(af_list))]
        print("delta", delta_list)
        load_delta = sum(delta_list)
        print("k={}, delta={}".format(num_partition, load_delta))
