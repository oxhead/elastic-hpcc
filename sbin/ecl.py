...
@click.pass_context
def run(ctx, target, ecl, dir, query):
    ...
    cmd = 'cd {}; {}/bin/ecl run --server {} --target {} {} {}'.format(program_dir, get_system_dir(ctx), eclagent_host, target, convert_to_query_xml(query), ecl)
    execute(cmd)
